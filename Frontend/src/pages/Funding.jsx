...
const Funding = () => {
  ...
  const getProfilePictureUrl = (profilePictureId) => {
    ...
    return `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/auth/profile-picture/${profilePictureId}`;
  };
  ...
  const fetchMyApplications = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/grant-applications/mine`,
        {
          headers: {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/funding/${selectedFundingId}`,
        {
          method: "DELETE",
        }
      );
