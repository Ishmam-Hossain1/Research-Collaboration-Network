...
const EquipmentDetail = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        // Load reviews from usage history
        try {
  ...
  const handleSubmitBooking = async (e) => {
    ...
    if (end < start) {
      setBookingError("End date must be at or after start date.");
      return;
    }
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50">
      <Navbar />
      <div className="flex items-center justify-center py-32 pt-24">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30">
      ...
      <div className="border-b border-slate-200 bg-white shadow-sm">
        <div className="mx-auto max-w-5xl px-4 py-6 pt-24">
          <button
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex-1 min-w-0">
              ...
              <h1 className="mt-1 text-3xl font-bold text-slate-900 leading-tight">{equipment.name}</h1>
            ...
            <div className="flex items-start gap-3">
              {equipment.isFree ? (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-3 text-center min-w-[120px]">
                ...
                <div className="rounded-2xl border border-blue-200 bg-blue-50 px-5 py-3 text-center min-w-[120px]">
                  <span className="flex items-center gap-1 text-blue-700 font-bold text-xl justify-center">{equipment.rentalPricePerDay} <span className="text-xs font-semibold uppercase tracking-wider opacity-70">tk</span></span>
      ...
      {/* Tabs */}
      <div className="border-b border-slate-200 bg-white sticky top-[78px] z-10">
          ...
          <div className="flex gap-0 overflow-x-auto no-scrollbar">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-1.5 border-b-2 px-5 py-4 text-sm font-semibold transition shrink-0 ${
                  activeTab === tab.id
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
            ...
            {activeTab === "overview" && (
              <>
                <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
                  <h2 className="mb-4 text-lg font-bold text-slate-800 flex items-center gap-2"><BookOpen size={16} /> Description</h2>
                  <p className="text-slate-600 leading-relaxed whitespace-pre-wrap">{equipment.description}</p>
                </div>

                {equipment.specifications && (
                  <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
                    <h2 className="mb-4 text-lg font-bold text-slate-800 flex items-center gap-2"><Info size={16} /> Specifications</h2>
                ...
                {equipment.usageInstructions && (
                  <div className="rounded-2xl border border-amber-100 bg-amber-50 p-7">
                    <h2 className="mb-4 text-lg font-bold text-amber-800 flex items-center gap-2"><AlertTriangle size={16} /> Usage Instructions</h2>
                ...
                {equipment.availabilitySchedule?.length > 0 && (
                  <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
                    <h2 className="mb-5 text-lg font-bold text-slate-800 flex items-center gap-2"><Clock size={16} /> Weekly Availability</h2>
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {equipment.availabilitySchedule.sort((a,b)=>a.dayOfWeek-b.dayOfWeek).map((slot, i) => (
                        <div key={i} className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3">
                          <p className="text-xs font-bold text-blue-700">{FULL_DAY_NAMES[slot.dayOfWeek]}</p>
                          <p className="text-xs text-blue-600 mt-0.5">{slot.startTime} – {slot.endTime}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
                ...
                {!currentUser ? (
                  <div className="rounded-xl bg-blue-50 p-6 text-center border border-blue-100">
                    <p className="text-slate-600 mb-4">You need to be logged in to book equipment.</p>
                    <button onClick={() => navigate("/login")} className="rounded-xl bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 transition shadow-md">
                      Log In
                    </button>
                  </div>
                ) : (
                  <form onSubmit={handleSubmitBooking} className="space-y-6">
                    <div className="grid gap-5 sm:grid-cols-2">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-700 focus:border-blue-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100 transition"
                          required
                        />
                      </div>
                      <div>
                    ...
                    <div>
                      <label className="mb-2 block text-sm font-semibold text-slate-700">Purpose / Research Goal <span className="text-red-500">*</span></label>
                      <textarea
                        name="purpose"
                        ...
                        className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-700 placeholder:text-slate-400 focus:border-blue-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100 transition"
                        required
                      />
                    </div>

                    <div>
                      <label className="mb-2 block text-sm font-semibold text-slate-700">Project Name (optional)</label>
                      <input
                        type="text"
                        name="projectName"
                        ...
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-700 placeholder:text-slate-400 focus:border-blue-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100 transition"
                      />
                    </div>

                    <div>
                      <label className="mb-2 block text-sm font-semibold text-slate-700">Notes to Owner (optional)</label>
                      <textarea
                        name="requesterNotes"
                    ...
                    {equipment.maxBookingDays && (
                      <p className="text-xs text-slate-400 flex items-center gap-1.5">
                        <Info size={14} /> Max booking duration: {equipment.maxBookingDays} days
                      </p>
                    )}

                    {bookingError && (
                      <div className="rounded-xl bg-red-50 p-4 text-sm text-red-600 flex items-center gap-2 border border-red-100">
                        <AlertTriangle size={16} /> {bookingError}
                      </div>
                    )}
                    {bookingSuccess && (
                      <div className="rounded-xl bg-emerald-50 p-4 text-sm text-emerald-700 border border-emerald-100 shadow-sm">{bookingSuccess}</div>
                    )}

                    <button
                      type="submit"
                      disabled={submitting}
                      className="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-3.5 text-sm font-bold text-white shadow-lg hover:bg-blue-700 disabled:opacity-60 transition"
                    >
                      {submitting ? (
              ...
              <div className="space-y-5">
                {reviews.length === 0 ? (
                  <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
                    ...
                    <p className="text-slate-500 font-medium">No reviews yet for this equipment.</p>
                  </div>
                ) : (
                  reviews.map((r) => (
                    <div key={r._id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:border-blue-200 transition">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className="h-9 w-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-bold text-xs uppercase">
                            {r.requester?.username?.[0] || "A"}
                          </div>
                          <div>
                            <p className="font-bold text-slate-800 text-sm">{r.requester?.username || "Anonymous Researcher"}</p>
                            <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">{formatDate(r.actualEndDate || r.endDate)}</p>
                          </div>
                        </div>
                        <StarRating value={r.rating} />
                      </div>
                      {r.reviewComment && <p className="text-sm text-slate-600 leading-relaxed italic">"{r.reviewComment}"</p>}
                      {r.projectName && <p className="mt-3 text-[11px] font-semibold text-blue-500 uppercase tracking-wide">Project: {r.projectName}</p>}
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="mb-4 text-sm font-bold text-slate-700 flex items-center gap-2"><User size={14} /> Equipment Owner</h3>
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-base shadow-inner">
                  {equipment.owner?.username?.[0]?.toUpperCase() || "?"}
                </div>
                <div className="min-w-0">
                  <p className="font-bold text-slate-800 truncate">{equipment.owner?.username}</p>
                  <p className="text-xs text-slate-400 truncate font-medium">{equipment.owner?.email}</p>
                </div>
              </div>
              {equipment.location && (
                <div className="mt-4 flex items-center gap-2 text-xs text-slate-500 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                  <MapPin size={13} className="shrink-0 text-blue-500" /> <span className="truncate">{equipment.location}</span>
                </div>
              )}
              {!isOwner && (
                <Link
                  to={`/researchers/${equipment.owner?._id}`}
                  className="mt-4 block text-center rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs font-bold text-blue-700 hover:bg-blue-100 transition shadow-sm"
                >
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
              <h3 className="text-sm font-bold text-slate-700 border-b border-slate-100 pb-2">Quick Info</h3>
              {[
                { label: "Category", val: equipment.category },
                { label: "Max duration", val: `${equipment.maxBookingDays} days` },
                { label: "Training", val: equipment.requiresTraining ? "Required" : "Not required" },
                { label: "Listed on", val: formatDate(equipment.createdAt) },
              ].map(({ label, val }) => (
            ...
            {!isOwner && equipment.status === "available" && activeTab !== "book" && (
              <button
                ...
                className="w-full rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 py-4 text-sm font-bold text-white shadow-lg shadow-blue-900/20 hover:from-blue-700 hover:to-indigo-700 transition-all hover:scale-[1.02]"
              >
                Book This Equipment
              </button>
            )}
            {!isOwner && equipment.status !== "available" && (
              <div className="rounded-2xl border border-red-100 bg-red-50 py-4 text-center text-sm font-bold text-red-600 shadow-sm">
