...
import {
  ...
  Wrench, Star, DollarSign, Gift, AlertTriangle, ChevronLeft, ChevronRight,
  Plus, MapPin, Tag, Zap,
} from "lucide-react";
...
const STATUS_STYLES = {
  available: { label: "Available", icon: <CheckCircle size={12} />, cls: "bg-emerald-100 text-emerald-700" },
  unavailable: { label: "Unavailable", icon: <XCircle size={12} />, cls: "bg-red-100 text-red-700" },
  maintenance: { label: "Maintenance", icon: <Wrench size={12} />, cls: "bg-amber-100 text-amber-700" },
};
    ...
    <Link to={`/equipment/${eq._id}`} className="group block">
      <div className="relative h-full rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-lg hover:border-blue-200">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <span className={`shrink-0 flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${st.cls}`}>
            {st.icon} {st.label}
          </span>
        </div>
          ...
          <div className="flex items-center gap-1 text-sm font-semibold">
            {eq.isFree ? (
              ...
              <span className="flex items-center gap-1 text-slate-700">
                <DollarSign size={14} />{eq.rentalPricePerDay}/day
              </span>
            )}
          </div>
        </div>
        ...
        <p className="mt-2 text-xs text-slate-400">
          By <span className="font-medium text-slate-600">{eq.owner?.username || "Unknown"}</span>
...
const EquipmentList = () => {
  ...
  }, [search, category, statusFilter, freeOnly, page]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50">
      ...
      {/* Hero Banner */}
      <div className="bg-gradient-to-r from-[#0f172a] via-[#102a4c] to-[#0b5c8e] px-6 py-12 text-white shadow-xl">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {currentUser && (
              <button
                ...
                className="flex items-center gap-2 rounded-xl bg-blue-500 px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-blue-400 transition"
              >
            ...
            {currentUser && (
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-5 py-2.5 text-sm font-semibold text-white hover:bg-white/20 transition backdrop-blur"
              >
                <Cpu size={16} /> My Equipment
              </button>
            )}
            {currentUser && (
              <button
      ...
      {/* Filters */}
      <div className="sticky top-[78px] z-10 border-b border-slate-200 bg-white/90 backdrop-blur-sm shadow-sm">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div className="flex flex-1 items-center gap-2 rounded-xl bg-slate-100 px-4 py-2.5 ring-1 ring-slate-200 focus-within:ring-blue-300 focus-within:bg-white transition">
              ...
              <input
                ...
                placeholder="Search equipment by name, description, tags…"
        ...
        {loading ? (
          ...
          <>
            <p className="mb-5 text-sm text-slate-500">
              Showing <span className="font-semibold text-slate-700">{equipment.length}</span> of{" "}
              <span className="font-semibold text-slate-700">{total}</span> equipment
            </p>
            ...
            {totalPages > 1 && (
              <div className="mt-10 flex items-center justify-center gap-3">
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 shadow-sm hover:bg-slate-50 disabled:opacity-40 transition"
                >
