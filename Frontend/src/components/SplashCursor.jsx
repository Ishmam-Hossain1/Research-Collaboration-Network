...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function maybeRunAutoMotion() {
      if (!autoMode.current) return;

      if (!autoPointer.initialized) {
        autoPointer.x = window.innerWidth * 0.25;
        autoPointer.y = window.innerHeight * 0.7;
        autoPointer.targetX = window.innerWidth * 0.75;
        autoPointer.targetY = window.innerHeight * 0.35;
        autoPointer.initialized = true;
      }

      const dxToTarget = autoPointer.targetX - autoPointer.x;
      const dyToTarget = autoPointer.targetY - autoPointer.y;
      const distance = Math.hypot(dxToTarget, dyToTarget);

      if (distance < 30) {
        autoPointer.targetX = Math.random() * window.innerWidth;
        autoPointer.targetY = Math.random() * window.innerHeight;
      }

      autoPointer.x += dxToTarget * 0.015 + (Math.random() - 0.5) * 1.5;
      autoPointer.y += dyToTarget * 0.015 + (Math.random() - 0.5) * 1.5;

      autoPointer.x = Math.max(0, Math.min(window.innerWidth, autoPointer.x));
      autoPointer.y = Math.max(0, Math.min(window.innerHeight, autoPointer.y));

      const pointer = pointers[0];
      const posX = scaleByPixelRatio(autoPointer.x);
      const posY = scaleByPixelRatio(autoPointer.y);
